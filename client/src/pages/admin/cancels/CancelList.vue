    ...
    <div class="admin-page">
        <div class="admin-content">
            <div class="admin-content__heading">
                <h3 v-show="!isTrashRoute">Quản lý phiếu hủy</h3>
                <h3 v-show="isTrashRoute">Quản lý thùng rác</h3>
                <router-link to="/admin/cancel/create" class="admin-content__create">Thêm phiếu hủy</router-link>
            </div>
            <div class="admin-content__table">
                <div class="admin-content__header d-flex align-items-center">
                    <router-link v-show="!isTrashRoute" to="/admin/cancel" class="admin-content__title-link">
                        <h4>Tất cả phiếu hủy</h4>
                    </router-link>
                    <router-link v-show="isTrashRoute" to="/admin/cancel/trash" class="admin-content__title-link">
                        <h4>Phiếu hủy đã xóa</h4>
                    </router-link>
                ...
                <CheckboxTable ref="checkboxTable" :items="cancels" @update:ids="handleUpdateIds">
                    <template #header>
                        <th scope="col">ID
                    ...
                    <template #body="{ item }">
                        ...
                        <td>
                            <template v-if="!isTrashRoute">
                                <router-link :to="'/admin/cancel/edit/' + item.id" class="fs-16 btn btn-primary">Sửa</router-link>&nbsp;
                        ...
                        <tr>
                            <td colspan="13" class="text-center">
                                <span v-show="isTrashRoute">
                                    Thùng rác trống.
                                    <router-link to="/admin/cancel">Danh sách phiếu hủy</router-link>
                                </span>
                                <span v-show="!isTrashRoute">
                                    Bạn chưa có phiếu hủy nào.
                                    <router-link to="/admin/cancel/create">Thêm phiếu hủy</router-link>
                                </span>
                            </td>
                        </tr>
                    </template>
                ...
                <div class="admin-content__table-footer">
                    <template v-if="!isTrashRoute">
                        <router-link to="/admin/cancel/trash">Thùng rác
    ...
    methods: {
        async fetchData() {
            try {
                const req = [
                    this.isTrashRoute
                        ? apiService.cancels.getTrashed(this.$route.query)
                        : apiService.cancels.get(this.$route.query)
                ];

                if (!this.isTrashRoute) {
                    req.push(
                        apiService.cancels.getTrashed(),
                    );
                }
                
                const res = await this.$swal.withLoading(Promise.all(req))

                this.cancels = res[0].data.data;
        ...
        async onDelete(id) {
            try {
                await apiService.cancels.delete(id)
        ...
        async onRestore(id) {
            try {
                await apiService.cancels.restore(id)
        ...
        async onForceDelete(id) {
            ...
            try {
                await apiService.cancels.forceDelete(id);
        ...
        async handleFormActions() {
            ...
            try {
                await apiService.cancels.handleFormActions({
                    action,
