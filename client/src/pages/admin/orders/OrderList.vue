...
export default {
  ...
  data() {
    return {
      ...
      orders: [],
      vouchers: [],
      selectedRefunded: this.$route.query.filterByRefunded || '', // Khởi tạo với giá trị từ query
    };
  },
  computed: {
    isTrashRoute() {
  ...
  methods: {
    ...
    async fetchData() {
      try {
        ...
        // Đồng bộ selectedRefunded với query sau khi fetch
        this.selectedRefunded = this.$route.query.filterByRefunded || '';
        if (this.$refs.selectedRefunded) {
          this.$refs.selectedRefunded.value = this.selectedRefunded;
        }
      } catch (error) {
        console.error(error);
      }
