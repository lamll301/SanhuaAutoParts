    ...
    <div class="admin-page">
        <div class="admin-content">
            <div class="admin-content__heading">
                <h3 v-show="!isTrashRoute">Quản lý phiếu thanh lý</h3>
                <h3 v-show="isTrashRoute">Quản lý thùng rác</h3>
                <router-link to="/admin/disposal/create" class="admin-content__create">Thêm phiếu thanh lý</router-link>
            </div>
            <div class="admin-content__table">
                <div class="admin-content__header d-flex align-items-center">
                    <router-link v-show="!isTrashRoute" to="/admin/disposal" class="admin-content__title-link">
                        <h4>Tất cả phiếu thanh lý</h4>
                    </router-link>
                    <router-link v-show="isTrashRoute" to="/admin/disposal/trash" class="admin-content__title-link">
                        <h4>Phiếu thanh lý đã xóa</h4>
                    </router-link>
                ...
                <CheckboxTable ref="checkboxTable" :items="disposals" @update:ids="handleUpdateIds">
                    <template #header>
                        <th scope="col">ID
                    ...
                    <template #body="{ item }">
                        ...
                        <td>
                            <template v-if="!isTrashRoute">
                                <router-link :to="'/admin/disposal/edit/' + item.id" class="fs-16 btn btn-primary">Sửa</router-link>&nbsp;
                        ...
                        <tr>
                            <td colspan="13" class="text-center">
                                <span v-show="isTrashRoute">
                                    Thùng rác trống.
                                    <router-link to="/admin/disposal">Danh sách phiếu thanh lý</router-link>
                                </span>
                                <span v-show="!isTrashRoute">
                                    Bạn chưa có phiếu thanh lý nào.
                                    <router-link to="/admin/disposal/create">Thêm phiếu thanh lý</router-link>
                                </span>
                            </td>
                        </tr>
                    </template>
                ...
                <div class="admin-content__table-footer">
                    <template v-if="!isTrashRoute">
                        <router-link to="/admin/disposal/trash">Thùng rác
    ...
    methods: {
        async fetchData() {
            try {
                const req = [
                    this.isTrashRoute
                        ? apiService.disposals.getTrashed(this.$route.query)
                        : apiService.disposals.get(this.$route.query)
                ];

                if (!this.isTrashRoute) {
                    req.push(
                        apiService.disposals.getTrashed(),
                    );
                }
                
                const res = await this.$swal.withLoading(Promise.all(req))

                this.disposals = res[0].data.data;
        ...
        async onDelete(id) {
            try {
                await apiService.disposals.delete(id)
        ...
        async onRestore(id) {
            try {
                await apiService.disposals.restore(id)
        ...
        async onForceDelete(id) {
            ...
            try {
                await apiService.disposals.forceDelete(id);
        ...
        async handleFormActions() {
            ...
            try {
                await apiService.disposals.handleFormActions({
                    action,
