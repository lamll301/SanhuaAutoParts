        ...
        <div class="admin-content" :class="{ 'loading-blur': isLoading }">
            ...
            <div class="admin-content__table">
                <div class="admin-content__header d-flex align-items-center">
                    <h4 v-show="!isTrashRoute">Tất cả tin tức</h4>
                    <h4 v-show="isTrashRoute">Tin tức đã xóa</h4>
    ...
    methods: {
        async fetchData() {
            ...
            const responseData = await handleApiCall(() => 
                this.$request.get(apiService.articles.get(this.$route.query, this.isTrashRoute))
            );
            ...
            if (!this.isTrashRoute) {
                const resDeleted = await handleApiCall(() => 
                    this.$request.get(apiService.articles.get({}, true))
                );
        ...
        async onDelete(id) {
            await handleApiCall(() => this.$request.delete(apiService.articles.delete(id)));
            await swalFire("Xóa thành công!", "Dữ liệu của bạn đã được xóa.", "success");
            await this.fetchData();
        },
        async onRestore(id) {
            await handleApiCall(() => this.$request.patch(apiService.articles.restore(id)));
        ...
        async onForceDelete(id) {
            ...
            await handleApiCall(() => this.$request.delete(apiService.articles.forceDelete(id)));
        ...
        async handleFormActions() {
            ...
            await handleApiCall(() => this.$request.post(apiService.articles.handleActions(), {
                action,
        ...
        getStatusLabel(status) {
            return statusService.getLabel('article', status);
        }
    }
}
</script>